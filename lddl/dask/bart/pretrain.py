...
def _get_sequences(
  ...
  def _aggregate_sentences(sentences):
    # Cutting sentences into chuncks that are close to target_seq_length
...
def save(pairs, path, output_format='parquet'):
  if output_format == 'parquet':
    ...
    pairs = pairs.map(lambda p: '{}'.format(
        p['sentences'],
    )).to_textfiles(os.path.join(path, '*.txt'))
  else:
